	...
	)
{
	if (code == MSGF_DIALOGBOX)
	{
		...
		{
			SendMessage(p->hwnd, p->message, p->wParam, p->lParam);
			return 1;
		}
	}
...
BOOL duCtrlManager::Create(HWND hWnd, TiXmlElement *pElement)
{
	...
	g_hookMsg = SetWindowsHookEx(WH_MSGFILTER, MessageProc, NULL, GetCurrentThreadId());
	...
	if (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST)
	{
		...
		for (i = 0;i < nLevelCount; i++)
		{
			...
			for (j = nCtrlCount - 1;j >= 0; j--)
			{
				...
				duPlugin* pParent = pPlugin;
				while (pParent = pParent->GetParent())
				{
					if (pParent->IsHookChildMouseMsg())
					{
						pPlugin = pParent;
						break;
					}
