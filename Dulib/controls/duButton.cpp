...
void WINAPI duButton::OnMouseLUp(POINT pt)
{
	...
	{
		BOOL bHandled = NotifyUser(DUM_BTNCLICK, NULL, NULL);
		if (Plugin_IsValid(this))//必须判断控件是否还有效，“关闭”按钮按下之后，NotifyUser使用SendMessage，
			//消息处理函数可能DestroyWindow，会导致duCtrlManager析构导致当前控件被析构。
		{
			m_bHandleClick = bHandled;
		}
	}
}
