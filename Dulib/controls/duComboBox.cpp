...
void WINAPI duComboBox::OnMouseLDown(POINT pt)
{
	...
	else
	{
		...
		//TODO:修复有些情况窗口不在初始设置的位置显示的问题
		::SetWindowPos(m_hWndListBox, HWND_TOPMOST, ptCombo.x, ptCombo.y, 0, 0, SWP_SHOWWINDOW|SWP_NOACTIVATE|SWP_NOSIZE);
...
void WINAPI duComboBox::OnKeyDown(UINT key, UINT nRepCnt, UINT nFlags)
{
	...
	else if (key == VK_RETURN)
	{
		if (!m_fReadOnly)
		{
			NotifyUser(DUM_KILLFOCUS, NULL, NULL);
		}
	}
	else
...
static LRESULT WINAPI ComboListBox_WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_CREATE:
		{
			...
			int nOldSel = nCurSel;
			LPCTSTR pszEditText = pComboBox->GetText();
			...
			for (i = 0; i < nItemCount; i++)
			{
				...
				if (_tcscmp(pszEditText, pNewItem->szText.c_str()) == 0)
				{
					nCurSel = i;
				}
			}
			pListBox->SetComboBoxListBox(FALSE);
			pListBox->SetSel(nCurSel);
			pListBox->SetComboBoxListBox(TRUE);
			pListBox->SetSel(nCurSel);
			pListBox->EnsureItemVisible(nCurSel);
			if (nCurSel != nOldSel)
			{
		...
		case WM_LBUTTONDOWN:
		{
			...
			duRect rcListBox;
			Plugin_GetRect(pListBox, &rcListBox);
			if (rcListBox.PtInRect(pt) && !pComboBox->IsReadOnly())
			{
				int nSel = pListBox->GetSel();
				if (nSel != -1)
				{
					LPCTSTR pszNew = pListBox->GetItemText(nSel);
					LPCTSTR pszOld = pComboBox->GetText();
					if (_tcscmp(pszOld, pszNew) != 0)
					{
						pComboBox->m_nCurSel = nSel;
						pComboBox->SetText(pszNew);
						pComboBox->SetEnd(TRUE);

						pComboBox->NotifyUser(DUM_COMBOSELCHANGE, nSel, NULL);
					}
				}
			}
		}
