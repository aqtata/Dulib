...
void MakeRectExt(int l, int t, int r, int b, int horzanchor, int vertanchor, int width, int height, RECT *lpInRect, RECT *lpOutRect)
{
	...
	if (l == DU_HCENTER) 
	{
		...
		{
			lpOutRect->left = rect.left + ( rect.Width() * (1-width/100.0) /2 );
			lpOutRect->right = lpOutRect->left + (rect.Width() * width/100.0);	
		} 
...
Rect_Vert:
	...
	if (t == DU_VCENTER) 
	{
		...
		{
			lpOutRect->top = rect.top + (rect.Height() * ( 1-height/100.0) ) /2;
			lpOutRect->bottom = lpOutRect->top + rect.Height() * height/100.0;
		} 
